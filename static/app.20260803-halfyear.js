...
const HISTORY_RANGES = {
  "3d": { days: 3, weekly: false },
  "7d": { days: 7, weekly: false },
  "30d": { days: 30, weekly: false },
  "3m": { months: 3, weekly: true },
  "6m": { months: 6, weekly: true },
};
...
function recordsForRange(records, rangeKey) {
  const config = HISTORY_RANGES[rangeKey] || HISTORY_RANGES["7d"];
  if (config.days) return records.slice(-config.days);
  if (!records.length) return [];
  const cutoff = parseHistoryDate(records.at(-1).date);
  cutoff.setUTCMonth(cutoff.getUTCMonth() - config.months);
  return records.filter((item) => parseHistoryDate(item.date) >= cutoff);
}
...
function aggregateWeekly(records) {
  ...
  return Array.from(groups, ([date, items]) => ({
    date,
    value: d3.sum(items, (item) => Number(item.value || 0)),
    period: `${items[0].date} 至 ${items.at(-1).date}`,
    days: items.length,
  }));
}
...
function renderHistory() {
  ...
  const config = HISTORY_RANGES[state.historyRange] || HISTORY_RANGES["7d"];
  const records = recordsForRange(selected.records, state.historyRange);
  const chartRecords = config.weekly ? aggregateWeekly(records) : records;
  ...
  document.querySelector("#history-count").textContent = records.length
    ? (config.weekly
      ? `${records.length} 个交易日 · ${chartRecords.length} 周`
      : `最近 ${records.length} 个交易日`)
    : "暂无记录";
  const scopeNote = state.historySelection === MARKET_HISTORY_KEY
    ? (state.historyPayload.scope_note || "")
    : "按交易日记录该板块当日累计主力净额。";
  document.querySelector("#history-note").textContent = config.weekly
    ? `${scopeNote} 当前柱状图按周合计，明细保留每日记录。`
    : scopeNote;
  ...
  renderHistoryChart(chartRecords, config.weekly);
}

function renderHistoryChart(records, weekly = false) {
  ...
  svg.selectAll("rect.history-bar").data(records).join("rect")
    ...
    .on("mouseenter", (event, item) => {
      const title = weekly ? `${item.date.slice(5)} 当周` : item.date;
      const detail = weekly ? `<div>${item.period} · ${item.days}个交易日</div>` : "";
      els.tooltip.innerHTML = `<b>${title}</b><strong class="${valueClass(item.value)}">${formatValue(item.value, 2)}</strong>${detail}`;
      els.tooltip.hidden = false;
      moveTooltip(event);
    })
    .on("mousemove", moveTooltip)
