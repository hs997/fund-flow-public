...
这里发布 A 股固定板块池的资金流快照，以及一个可安装的 Codex Skill。公开层只提供结果数据和读取工具，不包含抓取、视频渲染或 APP 源码。
...
GitHub 本身不是秒级实时行情后端。当前公开仓库采用已发布快照；若后续需要分钟级自动更新，应由私有采集服务写入公开数据接口，再由网站或 Skill 读取。
