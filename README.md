...
网页会直接读取公开仓库主分支的 `data/latest.json`，并每 60 秒自动检查一次公开快照是否更新。红色代表主力净流入，绿色代表主力净流出。公开端不依赖 GitHub API，避免关注者访问时触发匿名 API 限流；数据文件不走 GitHub Pages 静态缓存路径，避免公开页面长期停留在旧交易日。
...
GitHub Pages 是公开展示层，不是实时行情后端。当前网页每 60 秒读取公开仓库主分支的公开快照；分钟级持续更新由私有源码仓库里的发布器负责生成并推送 `data/latest.json`。GitHub Actions 定时任务不适合承诺严格 1 分钟更新，因此生产环境建议使用稳定在线的本机、VPS 或云服务器运行私有发布器，并用系统任务守护它。
